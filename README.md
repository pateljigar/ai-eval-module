...
**Tech Stack**: Python 3.11, LangGraph, LangChain, DeepEval, OpenAI GPT-4o-mini, GitHub Actions
...
[classify_claim]      → claim_type, urgency
    │
    ▼
[research_policy]     → policy_findings
    │
    ▼
...
**Modules 1–5 LangGraph pipeline:**
1. A claim is passed to the AusClaim AI triage agent
2. The agent classifies the claim, researches relevant policy, and produces a structured decision
3. DeepEval evaluates each node independently using GEval
4. GitHub Actions runs the full evaluator on every PR
