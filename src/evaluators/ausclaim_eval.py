...
faithfulness_metric = FaithfulnessMetric(model=model, threshold=0.7, include_reason=True, async_mode=False)

# Relevancy checks if the response directly addresses the question asked (relevance detection)
relevance_metric = AnswerRelevancyMetric(model=model, threshold=0.7, include_reason=True, async_mode=False)
...
def generate_response(question, retrieval_context):
    messages=[
        {"role": "system", "content": f"You are an AusClaim assistant. Only answer based on this context: {retrieval_context}"},
...
test_case_claim_lodging = LLMTestCase(
    input=questions[0],
    actual_output=generate_response(question=questions[0], retrieval_context=rag_knowledge_base),
    retrieval_context=rag_knowledge_base
)

test_case_windscreen_coverage = LLMTestCase(
    input=questions[1],
    actual_output=generate_response(question=questions[1], retrieval_context=rag_knowledge_base),
    retrieval_context=rag_knowledge_base
)
